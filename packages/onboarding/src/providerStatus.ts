...
import { readAllEnvKeys } from './envKeys'
...
export async function buildProviderStatusRows(options: ProviderStatusOptions = {}): Promise<ProviderStatusRow[]> {
  ...
  // Readiness env: an explicit env (tests, route handlers) always wins. Otherwise default to the real
  // process env OVERLAID with the local env-file keys, so a credential stored via the providers page (the
  // local OWLFOLIO_ENV_FILE) is honored by EVERY readiness surface — the Command Center masthead, the
  // active-mode indicator, and the run-readiness gate — not only the providers page that special-cased it.
  // Without this, a key present in the env file but not in process.env reads as "not connected" everywhere
  // except the providers page (the symptom this fixes).
  const readinessEnv: ProviderReadinessEnv = options.env
    ?? { ...(process.env as Record<string, string | undefined>), ...(await readAllEnvKeys()) }
  return Promise.all(getProviderCatalog().map(async (provider) => {
    const readiness = await getProviderReadiness(provider.provider_id, readinessEnv)
