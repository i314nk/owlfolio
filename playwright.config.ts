...
export default defineConfig({
  ...
  webServer: {
    // OWLFOLIO_RUN_LOG_DIR keeps e2e-spawned worker logs inside the playwright runtime dir — without
    // it they land in the real data/run-logs/ as confusing empty files (the test workers exit early).
    command: 'OWLFOLIO_TEST_MODE=playwright OWLFOLIO_PROJECT_DIR=$PWD OWLFOLIO_RUN_LOG_DIR=$PWD/.playwright-runtime/run-logs OWLFOLIO_APP_CONFIG_PATH=$PWD/.playwright-runtime/app-config.json OWLFOLIO_PERSONAL_LEDGER_PATH=$PWD/.playwright-runtime/personal-ledger.sqlite OWLFOLIO_CLAUDE_CREDENTIALS_PATH=$PWD/.playwright-runtime/missing-claude.json OWLFOLIO_CODEX_AUTH_PATH=$PWD/.playwright-runtime/missing-codex-auth.json ANTHROPIC_API_KEY= OPENAI_API_KEY= corepack pnpm dev',
