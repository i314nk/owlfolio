...
function isZeroStateRestore(restore: DataSafetyViewModel['restore']): boolean {
  const { files, ledgers, source_bundles, provider_reports } = restore.counts
  return (
    restore.path_rewrites.length === 0 &&
    files === 0 &&
    ledgers === 0 &&
    source_bundles === 0 &&
    provider_reports === 0
...
export function DataSafetyPanel({ dataSafety }: DataSafetyPanelProps) {
  const restoreRewriteRows = dataSafety.restore.path_rewrites.map((rewrite) => createElement(
    'li',
  ...
  const zeroStateRestore = isZeroStateRestore(dataSafety.restore)
    ...
    createElement(
      ...
      createElement(
        ...
        createElement(
          'p',
          { className: 'owl-muted-copy' },
          createElement('strong', null, 'To back up: '),
          'Run ',
          createElement('code', null, 'corepack pnpm worker -- --once --dry-run --define-defaults'),
          ' to verify state, then archive the paths listed under "Allowlisted runtime data" below. To restore, unpack the archive into the project workspace and re-authenticate providers — no web restore flow exists yet.',
        ),
      ),
      createElement(
        OwlCard,
      ...
      zeroStateRestore
        ? createElement(
          ...
          createElement(
            'p',
            { className: 'owl-muted-copy' },
            'No restore archive detected — backup/restore are operator-managed; see the runbook.',
          ),
        )
        : createElement(
          OwlCard,
