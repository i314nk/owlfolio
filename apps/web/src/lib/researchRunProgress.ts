 ...
 * actually waits through (quick-screen → circle → 7 lanes → synthesis → decision). Keep it a PURE function
...
export type ResolveRunProgressInput = {
  ...
  /** Count of recorded specialist findings — drives the live deep-dive "N/7" lane count. */
...
/**
 * The seven Buffett-Munger deep-dive specialist lanes (business_quality, moat, management, financial_quality,
 * shariah, risks, valuation). Hardcoded here to keep this module a pure, dependency-light presentation helper;
 * it mirrors `buffettMungerDeepDiveLanes.length` in `@owlfolio/workflow` (kept in sync by intent).
 */
export const DEEP_DIVE_LANE_TOTAL = 7
...
function buildStages(
  ...
  if (currentStage === 'done') {
    ...
    // recorded a finding. A full run completes with all seven lanes, so those read 'done'.
