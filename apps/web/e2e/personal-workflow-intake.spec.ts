...
test('personal-local mode can create the first research case from the command center', async ({ page }) => {
  ...
  await expect(page.locator('article').filter({ hasText: 'Microsoft FY2025 10-K' }).getByText('src_msft_10k_2025', { exact: true })).toBeVisible()
  await expect(page.locator('article').filter({ hasText: 'Microsoft 2025 Proxy Statement' }).getByText('src_msft_proxy_2025', { exact: true })).toBeVisible()
  await expect(page.locator('article').filter({ hasText: 'Microsoft Q1 2026 Shareholder Letter' }).getByText('src_msft_q1_2026', { exact: true })).toBeVisible()
