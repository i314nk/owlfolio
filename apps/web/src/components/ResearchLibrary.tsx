...
// Run-list staleness affordance: a compact "older engine" chip when the case's engine_version is present
// and differs from the current ENGINE_VERSION — so a stale run is visibly flagged, not silently trusted.
// Nothing is rendered for current or pre-versioning cases (kept minimal + owl-* consistent).
function olderEngineChip(researchCase: ResearchCaseProjection): ReactNode {
  const engineVersion = researchCase.valuation?.judgment?.engine_version
  if (engineVersion === undefined || engineVersion === ENGINE_VERSION) return null
  return createElement(
    'span',
    {
      'data-testid': 'older-engine-chip',
      title: `Generated by an older engine (${engineVersion})`,
      style: {
        ...monoLabel,
        color: 'var(--owl-color-gold-bright)',
        textTransform: 'none',
      ...
      createElement(
        ...
        createElement(
          ...
          researchCase.entity_name !== undefined
            ? createElement('span', { style: { color: 'var(--owl-color-muted)', flex: '0 1 auto', fontSize: 'var(--owl-text-sm)', minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } }, `— ${titleCaseEntityName(researchCase.entity_name)}`)
            : null,
        ),
