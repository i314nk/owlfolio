...
The current v2 repo has these catalog/runtime paths. **Update 2026-06-29:** the entire CLI/OAuth lane — Codex (`openai` / `openai-codex-cli`, app-server and one-shot exec), Claude CLI (`claude` / `claude-cli`), and Gemini CLI (`gemini-cli`) — was **excised**. **Update 2026-07-18 (PROVIDER CONSOLIDATION, owner-locked):** the direct API-key providers (`openai-api`, `anthropic-api`, `gemini-developer-api`) were **removed**, along with all model-tiering machinery (T1/T2/T3 roles, `OWLFOLIO_MODEL_ROLE_*` overrides, auto-tier assignment, dual-model cross-checks) — ONE configured model runs every stage of the analysis. Surviving providers are `mock-provider` (test-only) plus OpenRouter and the experimental `local` surface, on the function-calling tool-loop. The `openai`/`anthropic`/`google` **vendor** ids and their models are preserved (they label OpenRouter routes).
...
| `local` | Local OpenAI-compatible endpoint the user runs themselves (Ollama / vLLM; OpenAI-compatible adapter = `OpenRouterProvider` configured per-endpoint) | `experimental` | **UNSTABLE / EXPERIMENTAL / UNTESTED** — not exercised end-to-end by the owner; expect failures; fail-closed and never described as certified or live. |

**Grounding deny-bridge invariant (all surviving providers).** Native/provider-side web search is disabled (or risks-lane-only), and every grounded tool call routes through the harness-injected `ProviderToolExecutor`. Confirmed by inspection (2026-06-29): `OpenRouterProvider.createChatCompletion` sends only `model` + `messages` + `max_tokens` + the unified `reasoning` param + our function `tools`/`response_format` — **no OpenRouter `plugins` field and no `:online` model suffix**, so it cannot quietly web-search into a decision lane. The `local` surface is an `OpenRouterProvider` instance and inherits this body construction (with the `reasoning` param omitted — local endpoints reject unknown params). When configuring any provider, prove the native-disable took by inspecting the *sent* request, not just the config struct.
...
| `openai-api` (removed 2026-07-18) | `openai-api.latest.json` | historical evidence only | The direct OpenAI API provider was removed in the provider consolidation; the report remains dated evidence. |
| `gemini-developer-api` (removed 2026-07-18) | `gemini-developer-api.latest.json` | historical evidence only | The direct Gemini provider was removed in the provider consolidation; the report remains dated evidence. |
...
OpenRouter is the flagged frontier investment-grade candidate (the routed model carries the actual capability). The `mock-provider` is certified for the demo slice but is not an investment-grade candidate for live research; the `local` surface is not a candidate while untested.
