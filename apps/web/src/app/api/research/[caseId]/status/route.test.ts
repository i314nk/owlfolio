...
/** Append N specialist_finding_recorded events → an in-flight deep dive at N/7. */
...
describe('/api/research/[caseId]/status', () => {
  ...
  it('an in-progress deep dive → inProgress true with the right stage + lane count', async () => {
    ...
    expect(body.lanes).toEqual({ completed: 3, total: 7 })
    expect(body.label).toBe('Deep dive — 3/7 specialists')
