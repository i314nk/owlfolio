...
describe('Phase 6 S8a wiring conformance: the sell assembler is reachable from a LIVE path', () => {
  ...
  it('STRUCTURAL (a) no price-ALONE sell: the only price→sell path is evaluateValuationInverted with frozen_iv', () => {
    // The valuation-inverted call must be passed frozen_iv (it compares against the SIGN-OFF-FROZEN IV, never
    // a bare price move). We grep that the evaluateValuationInverted call site co-occurs with a frozen_iv arg.
    const code = stripComments(assemblerSrc)
    const invCall = /evaluateValuationInverted\(\s*\{[\s\S]*?frozen_iv[\s\S]*?\}\s*\)/
    expect(code, 'evaluateValuationInverted must be passed frozen_iv').toMatch(invCall)
  ...
  it('STRUCTURAL (c) frozen-IV-not-live: recordSellDecision reads frozen_iv from a projection, not a fresh recompute', () => {
    const slice = stripComments(recordSellDecisionSlice(webWorkflowSrc))
    // frozen_iv must be sourced from the nameLifecycle / admit projection row, NOT a fresh valuation recompute.
    expect(slice, 'recordSellDecision must read frozen_iv from a projection row').toMatch(/\.frozen_iv\b/)
    // It must NOT recompute a fair value / intrinsic value inline for the sell decision.
    expect(slice, 'recordSellDecision must not recompute fair_value for the sell decision').not.toMatch(/fair_value|computeFairValue|recomputeIv/i)
  })
