...
Owner's Manual v2 may observe and remind, but it must not autonomously create irreversible investment actions.
...
- Record dry-run/mock-safe observations for the scheduled task kinds (`re_review_check`, `watchlist_monitor`, `holdings_monitor`, `shariah_rescreen`, `portfolio_valuation_refresh`, `discovery_13f`, and the queue processors).
